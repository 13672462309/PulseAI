...
> 事件驱动的 A 股投资雷达——从热点中发现题材，用 AI 判定相关性，自动关联受益标的，把热点变成投资信号
...
- 🔍 **相关性复核** — 标题含关键词仍需 AI 判断；已判定话题不再重复进 AI（verifyRetry 精准重试）
- 🚫 **官网/公司页治理** — 品牌域名（含镜像）非新闻页、公司注册页、公司+商城标题规则拦截；官网新闻保留
- 🔁 **话题查重合并** — 7 天内同源同标题合并更新，不再重复展示（含存量 80 组合并）
- 🗑️ **关键词软删除** — 删除=停用并隐藏相关话题，同名重新添加自动恢复
- ⏱️ **2 小时自动扫描** — 默认扫描间隔 7200000ms，扩展词轮换同步
